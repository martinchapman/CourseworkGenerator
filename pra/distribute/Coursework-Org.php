...
class Coursework {
	...
	private function courseworkEntry($id, $target_folder, $coursework_id, $properties) {
		...
		if (!file_exists($target_folder . $coursework_id)) {
			...
			$index = fopen($target_folder . "/index.php", "w") or die("Error, please report to martin.chapman@kcl.ac.uk");
			
			fclose($index);
			
			$index = fopen($target_folder . $coursework_id . "/index.php", "w") or die("Error, please report to martin.chapman@kcl.ac.uk.");
			
			fclose($index);
			
		}
		
		$overviewfile = fopen($target_folder . $coursework_id . "/overview.csv", "a") or die("Error, please report to martin.chapman@kcl.ac.uk");
	...
	private function courseworkOverview($id, $coursework_id, $properties) {
		...
		if (flock($overviewfile, LOCK_EX)) { // do an exclusive lock
			...
			$logfile = fopen($target_folder . $coursework_id .  "/" . $id . "_log.txt", "w+") or die("Error, please report to martin.chapman@kcl.ac.uk.");
	...
	public function outputFile($id, $file_id, $coursework_id, $dictionary, $file, $output_folder, $message) {
		...
		if (!file_exists($output_folder)) {
			...
			$index = fopen($output_folder . "/index.php", "w") or die("Error, please report to martin.chapman@kcl.ac.uk.");
		...
		if (!file_exists($target_folder . $coursework_id)) {
			...
			$texfile = fopen($target_folder . $coursework_id . "/" . $file_id . ".tex", "w") or die("Error, please report to martin.chapman@kcl.ac.uk.");
			...
			if ($handle) {
			    ...
			    while (($line = fgets($handle)) !== false) {
					...
					if ( preg_match_all($pattern, $line, $matches, PREG_OFFSET_CAPTURE, 3) ) {	
			...
			$logfile = fopen($target_folder . $coursework_id .  "/" . $file_id . "_log.txt", "a") or die("Error, please report to martin.chapman@kcl.ac.uk.");
